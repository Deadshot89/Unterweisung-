...
app.http('learningSteps', {
  ...
  handler:async(request,context)=>{
    try{
      const ctx=await getAuthorizedContext(request);const pool=await getPool();const canEdit=hasRole(ctx,EDIT_ROLES);
      if(request.method==='GET'){
        ...
        const result=await pool.request().input('companyId',sql.NVarChar(80),ctx.companyId).input('instructionTypeId',sql.NVarChar(80),instructionTypeId).input('language',sql.NVarChar(10),lang)
          .query(`SELECT s.id,s.instructionTypeId,s.language,s.sortOrder,s.title,s.body,s.imageFileId,s.status,s.reviewedBy,s.reviewedAt,s.createdAt,s.updatedAt,
      ...
      if(request.method==='POST'){
        ...
        const id=clean(body.id,80)||`step-${uuidv4()}`;
        await pool.request().input('id',sql.NVarChar(80),id).input('companyId',sql.NVarChar(80),ctx.companyId).input('instructionTypeId',sql.NVarChar(80),instructionTypeId)
          ...
          .query(`INSERT INTO InstructionLearningSteps(id,companyId,instructionTypeId,language,sortOrder,title,body,imageFileId,status,createdBy)
                  VALUES(@id,@companyId,@instructionTypeId,@language,@sortOrder,@title,@body,@imageFileId,'draft',@createdBy)`);
        await linkImage(pool,ctx.companyId,imageFileId,id);await writeAudit(pool,ctx,'learningStep.created','learningStep',id,{instructionTypeId,title});return json({id,status:'draft'},201);
      }
      ...
      const publish=body.status==='published';const unpublish=body.status==='draft';
      await pool.request().input('companyId',sql.NVarChar(80),ctx.companyId).input('id',sql.NVarChar(80),id).input('title',sql.NVarChar(240),body.title===undefined?null:clean(body.title,240)).input('body',sql.NVarChar(sql.MAX),body.body===undefined?null:clean(body.body,20000))
        ...
        .query(`UPDATE InstructionLearningSteps SET title=COALESCE(@title,title),body=CASE WHEN @body IS NULL THEN body ELSE @body END,sortOrder=COALESCE(@sortOrder,sortOrder),
                imageFileId=CASE WHEN @imageSpecified=1 THEN @imageFileId ELSE imageFileId END,language=COALESCE(@language,language),status=CASE WHEN @publish=1 THEN 'published' WHEN @unpublish=1 THEN 'draft' ELSE status END,
