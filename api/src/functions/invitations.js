...
app.http('invitations',{methods:['GET','POST','PATCH'],authLevel:'anonymous',route:'invitations/{id?}',handler:async(request,context)=>{
  try{
    ...
    if(request.method==='GET'){
      ...
      if(access.mode==='self'||access.mode==='team'){const employeeScope=bindEmployeeScope(req,access,'vi.employeeId','inviteEmployee');scope=` AND ${employeeScope}`;}
    ...
    if(request.method==='POST'){
      ...
      if(access.mode==='team'){if(!body.employeeId)return badRequest('Line Manager können externe Links nur für zugewiesene Mitarbeiter erstellen.');requireEmployeeTarget(access,body.employeeId);}
