...
function setProfessionalVersion(){
  ...
  if(version && version.textContent !== APP_RELEASE_VERSION) version.textContent = APP_RELEASE_VERSION;
  if(document.body.dataset.professionalSuite !== PROFESSIONAL_SUITE_VERSION) document.body.dataset.professionalSuite = PROFESSIONAL_SUITE_VERSION;
  if(document.body.dataset.appRelease !== APP_RELEASE_VERSION) document.body.dataset.appRelease = APP_RELEASE_VERSION;
}
function navButton(viewId){ return document.querySelector(`.primary-tabs button[data-view="${viewId}"]`); }
function applyNavigationMeta(){Object.entries(NAV_META).forEach(([viewId,meta])=>{const button=navButton(viewId);if(!button)return;if(button.dataset.navIcon!==(meta.icon||''))button.dataset.navIcon=meta.icon||'';const label=meta.label||button.textContent.trim();if(button.textContent!==label)button.textContent=label;if(button.getAttribute('title')!==label)button.setAttribute('title',label);});}
function updateNavigationGroups(){
  const nav=document.querySelector('.primary-tabs');if(!nav)return;nav.classList.add('pro-navigation');applyNavigationMeta();if(nav.dataset.groupedVersion===PROFESSIONAL_SUITE_VERSION)return;nav.querySelectorAll('.nav-group-title').forEach(el=>el.remove());const orderedButtons=[];
  NAV_GROUPS.forEach(group=>{const title=document.createElement('span');title.className='nav-group-title';title.dataset.navGroup=group.key;title.textContent=group.label;nav.appendChild(title);group.views.forEach(viewId=>{const button=navButton(viewId);if(!button)return;if(button.dataset.navGroup!==group.key)button.dataset.navGroup=group.key;nav.appendChild(button);orderedButtons.push(button);});});
...
if(typeof setView==='function'){const originalSetViewForProfessionalSuite=setView;setView=function(id){const result=originalSetViewForProfessionalSuite(id);scheduleProfessionalSuite();return result;};}
if(typeof render==='function'){const originalRenderForProfessionalSuite=render;render=function(id){const result=originalRenderForProfessionalSuite(id);scheduleProfessionalSuite();return result;};}
document.addEventListener('click',event=>{if(event.target.closest('.primary-tabs button[data-view]'))scheduleProfessionalSuite();});
document.addEventListener('DOMContentLoaded',scheduleProfessionalSuite);window.addEventListener('load',scheduleProfessionalSuite);
window.PROFESSIONAL_SUITE_VERSION=PROFESSIONAL_SUITE_VERSION;window.APP_RELEASE_VERSION=APP_RELEASE_VERSION;window.NAV_GROUPS=NAV_GROUPS;window.NAV_META=NAV_META;window.applyProfessionalSuite=applyProfessionalSuite;window.scheduleProfessionalSuite=scheduleProfessionalSuite;window.updateNavigationGroups=updateNavigationGroups;
scheduleProfessionalSuite();
