...
let typeHandler;
const typeContext=vm.createContext({app:{http(name,definition){typeHandler=definition.handler;}},getPool:async()=>({request(){const params={};return {input(name,type,value){params[name]=value;return this;},async query(query){if(query.startsWith('UPDATE')) updatedDescription=params.description;return {recordset:[{id:'type-a'}]};}};}}),sql:fakeSql,getAuthorizedContext:async()=>({...ctx,isAuthenticated:true,roles:[Roles.COMPANY_ADMIN]}),assertRole,Roles,learningContentSchemaReady:async()=>false,parseKeyPoints:()=>[],serialiseKeyPoints:value=>JSON.stringify(value),requireLearningContentSchema:async()=>{},json:value=>value,badRequest:message=>({error:message}),serverError:error=>{throw error;},writeAudit:async()=>{}});
