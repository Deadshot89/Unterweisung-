...
const portal = readFileSync('frontend/employee-portal-v37.js', 'utf8');
...
assert.match(api, /methods:\s*\['GET',\s*'POST',\s*'PATCH'\]/, 'InstructionTypes API muss GET, POST und PATCH unterstützen.');
assert.match(api, /Roles\.SYSTEM_ADMIN,\s*Roles\.COMPANY_ADMIN,\s*Roles\.HSE/, 'Schreiben muss System Admin, Company Admin und HSE erlauben.');
...
assert.match(api, /deliveryMode/, 'API muss Online- und Praxisdurchführung unterscheiden.');
assert.match(api, /testRequired/, 'API muss den Abschlusstest konfigurieren können.');
...
assert.match(portal, /portalDeliveryMode/, 'Erweiterte Oberfläche muss Abschlussart verwalten können.');
