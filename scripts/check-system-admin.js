...
assert.match(auth, /base\.requestedCompanyId \|\| null/, 'System Admin darf ohne bewusste Auswahl nicht in eine Default-Firma fallen.');
assert.match(auth, /activeCompanyExists/, 'Eine explizit gewählte System-Admin-Firma muss serverseitig als aktiv validiert werden.');
assert.match(auth, /companyId:null/, 'System Admin muss einen authentifizierten Auswahlzustand ohne Firmenkontext unterstützen.');
